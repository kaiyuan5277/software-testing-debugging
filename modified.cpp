...
struct Student {
    ...
    {
        strcpy_s(name, name_);//modified: change strcpy() to strcpy_s()
        strcpy_s(USF_ID, USF_ID_);
        strcpy_s(email, email_);
...
vector<Student> readDataFile(string filename, bool opened)//modified
{
        ...
        //If file opened, read the data line by line
    {
        ...
        while (getline(dataFile, line))   //while still getting the line
        {
            ...
            getline(ss, data, ',');
            strcpy_s(dataName, data.c_str());//modified: change strcpy() to strcpy_s()
            if (data.length() > 40)skip = true;
            getline(ss, data, ',');
            strcpy_s(dataID, data.c_str());//modified: change strcpy() to strcpy_s()
            if (data.length() != 10)skip = true;
            getline(ss, data, ',');
            strcpy_s(dataEmail, data.c_str());//modified: change strcpy() to strcpy_s()
            if (data.length() > 40)skip = true;
            getline(ss, data, ',');
...
void addStudent(vector<Student> &studentVector)//modified: add symbol &, pass by reference
{
    ...
    strcpy_s(name, input.c_str());//modified: change strcpy() to strcpy_s()
    input = stringInputCheck("his/her USF ID [U-12345678]: ", 10);
    strcpy_s(ID, input.c_str());//modified: change strcpy() to strcpy_s()
    input = stringInputCheck("his/her email [up to 40 characters]: ", 40);
    strcpy_s(email, input.c_str());//modified: change strcpy() to strcpy_s()
...
void printAllStudents(vector<Student> studentData)
{
    ...
    for (unsigned int i = 0; i < studentData.size(); i++)//modified: change i <= to i <
    {
        cout << " " << left << setw(40) << studentData[i].name << setw(18) << studentData[i].USF_ID << setw(40) << studentData[i].email << setw(22) <<
            ...
            ((studentData[i].essayGrade == -1) ? '-' : studentData[i].essayGrade) << setw(22) <<
            ((studentData[i].projectGrade == -1) ? '-' : studentData[i].projectGrade) << endl;
    }
    cout << endl;
}
